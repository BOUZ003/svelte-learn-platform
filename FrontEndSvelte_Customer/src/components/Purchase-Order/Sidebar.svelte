<script lang="ts">
  ...
  function redirectToMyLearning() {
    goto(`/my-learning/${userId}`); 
  }

  function redirectToPurchaseOrder() {
    goto(`/purchase-order/${userId}`); 
  }
</script>
...
<nav class="nav flex-column">
  ...
  <div class="nav-item">
    <a
      class="nav-link text-dark mb-1 d-flex align-items-center justify-content-between"
      href="javascript:void(0)"
      on:click={redirectToPurchaseOrder}
      aria-label="Settings"
       style="border-left: 3px solid #0ddd0d;"
    >
      <i class="fa-solid fa-list" style="color: #0ddd0d;"></i>
      <span style="color: #0ddd0d;">
        {#if !isCollapsed}Purchase Order{/if}
      </span>
      {#if !isCollapsed}
        <i></i>
      {/if}
